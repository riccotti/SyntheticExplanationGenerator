    ...
    def apply_cutpoints(self, out_data_path=None, out_bins_path=None):
        ...
        if out_bins_path:
            with open(out_bins_path, 'w') as bins_file:
                bins_file.write('Description of bins in file: %s' % out_data_path)
                for attr in self._features:
...
def main(argv):
    ...
    try:
        parameters, _ = getopt.getopt(argv, shortopts='', longopts=['in_path=', 'out_path=', 'features=',
                                                                    'class_label=', 'return_bins'])
    except:
    ...
    discretizer = MDLP_Discretizer(dataset=data, class_label=class_label, features=features,
