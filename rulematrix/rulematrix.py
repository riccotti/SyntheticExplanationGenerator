...
from sample import create_sampler, check_input_constraints
...
class Surrogate(object):
    ...
    def explain(self, x, m=None):
        rule_list = self.student
        if m is None:
            m = rule_list.n_features
        decision_path = rule_list.decision_path(x.reshape(1, -1))
        features_used = np.zeros(m).astype(int)
        for rule, is_used in zip(rule_list._rule_list, decision_path):
            if is_used:
                for c in rule[0]:
